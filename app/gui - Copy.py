...
class ImagePrepApp(ttk.Frame):
    ...
    def _build_root_layout(self):
        ...
        # sidebar
        self.sidebar = ttk.Frame(body, width=200); self.sidebar.pack(side="left", fill="y"); self.sidebar.pack_propagate(False)
    ...
    def _build_split_page(self, parent):
        ...
        ttk.Entry(io_frame, textvariable=self.input_path, width=80).grid(row=0, column=1, sticky="we", padx=6, pady=6)
        b1 = ttk.Button(io_frame, text="Browse...", command=self._pick_input); b1.grid(row=0, column=2, padx=6, pady=6)
        ...
        ttk.Entry(io_frame, textvariable=self.input_path_t2, width=80).grid(row=1, column=1, sticky="we", padx=6, pady=6)
        ...
        ttk.Entry(io_frame, textvariable=self.output_dir, width=80).grid(row=2, column=1, sticky="we", padx=6, pady=6)
        b3 = ttk.Button(io_frame, text="Browse...", command=self._pick_output); b3.grid(row=2, column=2, padx=6, pady=6)
        ...
        ttk.Entry(tiling, textvariable=self.tile_size, width=10).grid(row=0, column=1, sticky="w", padx=6, pady=6)

        ttk.Label(tiling, text="Overlap (%):").grid(row=0, column=2, sticky="w", padx=6, pady=6)
        ttk.Entry(tiling, textvariable=self.overlap_pct, width=10).grid(row=0, column=3, sticky="w", padx=6, pady=6)
        ...
        ttk.Entry(tiling, textvariable=self.t1_base, width=28).grid(row=1, column=1, sticky="w", padx=6, pady=6)
        Tooltip(tiling, "لو فاضي: هنستخدم اسم ملف T1")

        ttk.Label(tiling, text="Base (T2):").grid(row=1, column=2, sticky="e", padx=6, pady=6)
        ttk.Entry(tiling, textvariable=self.t2_base, width=28).grid(row=1, column=3, sticky="w", padx=6, pady=6)
        Tooltip(tiling, "لو فاضي: هنستخدم Base(T1) + _T2")

        ttk.Label(tiling, text="Filename Pattern:").grid(row=2, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(tiling, textvariable=self.name_pattern, width=60).grid(row=2, column=1, columnspan=5, sticky="we", padx=6, pady=6)
        Tooltip(tiling, "Tokens: {base} {y} {x} {row} {col} {i} {ext}  — مثال: {base}_tile_{y}_{x}")
        ...
        self.band_box = ttk.Frame(bands); self.band_box.pack(fill="x", padx=8, pady=8)
        # نسيب الاختيار يتعبّى بعد اختيار T1 (حسب الميتاداتا)
        ...
        ttk.Entry(meta, textvariable=self.scene_id, width=28).grid(row=0, column=1, sticky="w", padx=6, pady=6)

        ttk.Label(meta, text="Fold:").grid(row=0, column=2, sticky="w", padx=6, pady=6)
        ttk.Combobox(meta, textvariable=self.fold, values=["train", "val", "test"], state="readonly", width=12)\
            .grid(row=0, column=3, sticky="w", padx=6, pady=6)

        ttk.Label(meta, text="Label (optional):").grid(row=1, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(meta, textvariable=self.label_path, width=60).grid(row=1, column=1, columnspan=2, sticky="we", padx=6, pady=6)
        ttk.Checkbutton(meta, text="Write Parquet alongside CSV", variable=self.write_parquet).grid(row=1, column=3, sticky="w", padx=6, pady=6)
        meta.columnconfigure(1, weight=1); meta.columnconfigure(2, weight=1)

        # Actions
        actions = ttk.Frame(page); actions.pack(fill="x", padx=10, pady=(0, 10))
        ttk.Button(actions, text="Preview First Tile", command=self._preview_from_split).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="Split Image", command=self._do_split_threaded).pack(side="left", padx=(0, 8))
        ttk.Button(actions, text="Reset", command=self._reset_all).pack(side="left")
    ...
    def _build_merge_page(self, parent):
        ...
        src = ttk.LabelFrame(page, text="Source"); src.pack(fill="x", padx=10, pady=10)
        ttk.Label(src, text="Tiles Folder:").grid(row=0, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(src, textvariable=self.tiles_dir, width=80).grid(row=0, column=1, sticky="we", padx=6, pady=6)
        ttk.Button(src, text="Browse...", command=self._pick_tiles_dir).grid(row=0, column=2, padx=6, pady=6)

        ttk.Label(src, text="Output File:").grid(row=1, column=0, sticky="w", padx=6, pady=6)
        ttk.Entry(src, textvariable=self.merge_out_path, width=80).grid(row=1, column=1, sticky="we", padx=6, pady=6)
        ttk.Button(src, text="Save As...", command=self._pick_merge_out).grid(row=1, column=2, padx=6, pady=6)
        src.columnconfigure(1, weight=1)

        ops = ttk.LabelFrame(page, text="Operations"); ops.pack(fill="x", padx=10, pady=10)
        ttk.Button(ops, text="Scan & Estimate Size", command=self._estimate_merge).grid(row=0, column=0, padx=6, pady=6)
        self.merge_estimate_lbl = ttk.Label(ops, text=""); self.merge_estimate_lbl.grid(row=0, column=1, padx=6, pady=6, sticky="w")
        ttk.Button(ops, text="Merge from Folder", command=self._do_merge_threaded).grid(row=1, column=0, padx=6, pady=6)
        ttk.Button(ops, text="Merge from manifest.csv", command=self._merge_from_manifest_threaded).grid(row=1, column=1, padx=6, pady=6)

        ttk.Label(ops, text="(optional) Fold filter:").grid(row=1, column=2, sticky="e", padx=6, pady=6)
        ttk.Combobox(ops, textvariable=self.merge_fold_var, values=["", "train", "val", "test"], state="readonly", width=10)\
    ...
    def _build_preview_page(self, parent):
        page = ttk.Frame(parent)
        ttk.Label(page, text="Quick Preview (uses Splitter settings)", font=("Segoe UI Semibold", 11))\
            .pack(anchor="w", padx=10, pady=(10, 6))
        ttk.Button(page, text="Preview First Tile", command=self._preview_from_split)\
    ...
    def _build_help_page(self, parent):
        ...
        help_text = """
SPLITTER
- اختار T1 و (اختياري) T2 تحتها مباشرةً، وحدد فولدر الإخراج.
- اختار Tile Size و Overlap و Format.
- Base (T1/T2) لتغيير البادئة في أسماء التايلز (لو فاضي: T1=اسم الملف، T2=Base(T1)+_T2).
- Filename Pattern يدعم {base} {y} {x} {row} {col} {i} {ext}.
- البرنامج هيكتب T1/ و T2/ داخل فولدر الإخراج؛ وكل واحد ليه manifest.csv خاص.
"""
        box.insert("1.0", help_text.strip()); box.configure(state="disabled")
        return page
    ...
    def _preview_from_split(self):
        ...
        try:
            with Image.open(self.input_path.get()) as im:
                ...
                top = tk.Toplevel(self.master); top.title("Preview"); top.resizable(False, False)
                photo = ImageTk.PhotoImage(crop)
                lbl = ttk.Label(top, image=photo); lbl.image = photo; lbl.pack(padx=8, pady=8)
    ...
    # ---------------- Reset ----------------
    def _reset_all(self):
        self.input_path.set(""); self.input_path_t2.set(""); self.output_dir.set("")
        self.tile_size.set(512); self.overlap_pct.set(0.0); self.extension.set(".png")
        self.t1_base.set(""); self.t2_base.set("")
        self.name_pattern.set("{base}_tile_{y}_{x}")
        self.scene_id.set(""); self.fold.set("train"); self.label_path.set(""); self.write_parquet.set(False)
        for w in self.band_box.winfo_children():
            w.destroy()
        self.selected_bands_vars.clear()
        self.status.config(text="Reset to defaults.")
        # امسح الإعدادات على القرص كمان
        try:
    ...
    def _do_split_worker(self):
        ...
        try:
            result = split_large_image(
                ...
                name_pattern_t2=None,  # بنستخدم نفس الـ pattern لكن بقاعدة Base مختلفة
                write_manifest=True,
        ...
        try:
            ...
            for i, fp in enumerate(files):
                ...
                with Image.open(fp) as im:
                    ...
                    if not m:
                        pd.step(1); continue
                    y = int(m.group(1)); x = int(m.group(2))
                    canvas.paste(im, (x, y))
                if i % 16 == 0:
    ...
    def _merge_from_manifest_worker(self):
        ...
        try:
            with open(manifest_path, "r", encoding="utf-8") as f:
                ...
                # اسم عمود الصورة: يقبل t1_path أو t2_path أو path
                img_col = None
        ...
        try:
            max_x2 = 0; max_y2 = 0; first_img = None
            for r in rows:
                x0 = int(float(r["x0"])); y0 = int(float(r["y0"]))
                w = int(float(r["w"]));   h = int(float(r["h"]))
                max_x2 = max(max_x2, x0 + w); max_y2 = max(max_y2, y0 + h)
    ...
    def _on_close(self):
        # احتفظ بالإعدادات، ولو عايز تمسحها تلقائيًا uncomment السطرين الجايين
        # try:
